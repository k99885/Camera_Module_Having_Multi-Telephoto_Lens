...
- **영상 처리 기술**: 여러 대의 망원카메라로 취득한 메가급 영상을 영상 처리 기술로 병합하여 넓은 화각의 영상을 생성.
- **결과**: 병합된 영상은 각 카메라 모듈의 메가급 영상의 합으로 이루어지며, 단일 카메라보다 작은 사이즈의 다중 망원카메라를 이용하여 높은 해상도의 영상을 취득할 수 있음.
